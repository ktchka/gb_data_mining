...
class GbBlogParse:
    def __init__(self, start_url, database:Database):
        ...
        # внутри множества(set) только хэшируемые данные
        self.done_urls = set()
        # порождаем задачу для такого юрла, чтоб отработаала такая-то функция
        self.tasks = [self.get_task(self.start_url, self.parse_feed)]
        self.done_urls.add(self.start_url)

    def get_task(self, url, callback):
        # запираем callback функцию и юрл, с которым она должна выполниться
        def task():
    ...
    def parse_post(self, url, soup):
        #  можно использовать template подход из урока 2
        # image остается для дз
    ...
    def parse_feed(self, url, soup):
        ...
        pag_urls = set(urljoin(url, href.attrs.get('href'))
                       ...
                       href.attrs.get('href'))
        # порождаем задачу, если их еще не была
